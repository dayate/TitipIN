<script lang="ts">
	...
	function getActionColor(action: string): string {
		...
		if (action.includes('rejected') || action.includes('cancelled') || action.includes('demoted')) {
			return 'text-red-500';
		}
...
<div class="container max-w-4xl mx-auto p-4 space-y-6">
	...
	<!-- Info -->
	<div class="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center gap-3">
		<FileText class="h-5 w-5 text-blue-500" />
		<p class="text-sm">
			Audit log mencatat semua perubahan penting di lapak Anda untuk transparansi dan
			accountability.
		</p>
	</div>
					...
					<div class="flex items-start gap-4">
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2 mb-1 flex-wrap">
								...
								<span class="text-xs px-2 py-0.5 rounded-full bg-muted">{log.entityType}</span>
	...
	{:else}
		<Card>
			<div class="py-8 text-center">
				...
				<p class="text-muted-foreground">Audit log akan muncul setelah ada aktivitas di lapak</p>
			</div>
		</Card>
	{/if}
</div>
