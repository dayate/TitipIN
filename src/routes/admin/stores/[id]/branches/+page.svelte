<script lang="ts">
	...
	async function updateBranch() {
		...
		try {
			const res = await fetch(`/api/stores/${data.store.id}/branches/${editingBranch.id}`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ name, address, phone, isMain })
			});
	...
	async function deleteBranch(branchId: number) {
		...
		try {
			const res = await fetch(`/api/stores/${data.store.id}/branches/${branchId}`, {
				method: 'DELETE'
			});
	...
	async function toggleActive(branch: (typeof data.branches)[0]) {
		try {
			const res = await fetch(`/api/stores/${data.store.id}/branches/${branch.id}`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
...
<div class="mx-auto max-w-4xl space-y-6">
	...
	{#if data.branches.length === 0}
		<Card class="text-center py-12">
			...
			<h3 class="text-lg font-semibold text-foreground mb-2">Belum Ada Cabang</h3>
			<p class="text-muted-foreground mb-4">Tambah cabang untuk mengelola lokasi lapak Anda</p>
			<Button
				onclick={() => {
					resetForm();
					showAddModal = true;
...
{#if showAddModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog">
		<Card class="w-full max-w-md">
			...
			<div class="space-y-4">
				<div>
					<label for="name" class="text-sm font-medium text-foreground">Nama Cabang *</label>
					<input
						id="name"
						type="text"
						bind:value={name}
				...
				<div>
					<label for="phone" class="text-sm font-medium text-foreground">Telepon</label>
					<input
						id="phone"
						type="text"
						bind:value={phone}
...
{#if showEditModal && editingBranch}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog">
		<Card class="w-full max-w-md">
			...
			<div class="space-y-4">
				<div>
					<label for="edit-name" class="text-sm font-medium text-foreground">Nama Cabang *</label>
					<input
						id="edit-name"
						type="text"
						bind:value={name}
				...
				<div>
					<label for="edit-address" class="text-sm font-medium text-foreground">Alamat *</label>
					<textarea
						id="edit-address"
						bind:value={address}
						rows="2"
				...
				<div>
					<label for="edit-phone" class="text-sm font-medium text-foreground">Telepon</label>
					<input
						id="edit-phone"
						type="text"
						bind:value={phone}
