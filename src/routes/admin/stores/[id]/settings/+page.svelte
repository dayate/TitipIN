	...
	>
			...
			<div class="space-y-3">
				...
				<div class="grid grid-cols-2 gap-3">
					...
					>
						...
						{/if}
						<div
							class="flex h-12 w-12 items-center justify-center rounded-xl {visibility === 'public'
								? 'bg-primary/20'
								: 'bg-muted'}"
						>
							<Globe
								class="h-6 w-6 {visibility === 'public' ? 'text-primary' : 'text-muted-foreground'}"
							/>
						</div>
						<div class="text-center">
							<p
								class="font-semibold {visibility === 'public' ? 'text-primary' : 'text-foreground'}"
							>
								Publik
							</p>
					...
					>
						...
						{/if}
						<div
							class="flex h-12 w-12 items-center justify-center rounded-xl {visibility === 'private'
								? 'bg-primary/20'
								: 'bg-muted'}"
						>
							<Lock
		...
		<div class="rounded-2xl border border-border bg-card p-5 space-y-5 shadow-sm">
			<div class="flex items-center gap-3 pb-3 border-b border-border">
				...
				<div>
					<h2 class="text-lg font-semibold text-foreground">Pengaturan Cut-off</h2>
					<p class="text-xs text-muted-foreground">Batas waktu input transaksi harian</p>
				</div>
			</div>

			<!-- Cutoff Time -->
			<div class="space-y-2">
				<label for="cutoffTime" class="text-sm font-medium text-foreground">Waktu Cut-off</label>
			...
			<!-- Grace Period -->
			<div class="space-y-2">
				<label for="cutoffGracePeriod" class="text-sm font-medium text-foreground">
			...
			{#if autoCancelEnabled}
				<div class="flex items-start gap-3 rounded-xl bg-muted/50 p-4">
					...
					<div class="text-xs text-muted-foreground">
						<p class="font-medium text-foreground mb-1">Cara kerja Auto-Cancel:</p>
						<ul class="list-disc pl-4 space-y-1">
							<li>Transaksi draft yang belum disetor akan dibatalkan otomatis</li>
