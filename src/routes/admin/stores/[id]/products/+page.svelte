<script lang="ts">
	...
	let filteredBySupplier = $derived(() => {
		return data.productsBySupplier.map(group => ({
			supplier: group.supplier,
			products: activeTab === 'all'
				? group.products
	...
	function getStatusBadge(status: string) {
		switch (status) {
			case 'approved':
				return { class: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', label: 'Disetujui', Icon: CheckCircle2 };
			case 'rejected':
...
<div class="space-y-6">
	<div>
		<a href="/admin/stores/{data.store.id}" class="mb-2 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
	...
	<!-- Success Message -->
	{#if form?.success}
		<div class="flex items-center gap-2 rounded-lg p-3 text-sm {form.action === 'approve' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : form.action === 'delete' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}">
					...
					{#if expandedSuppliers.has(supplier.id)}
						<div class="border-t border-border p-4 space-y-4">
							{#each products as { product }}
								...
								<div class="flex flex-col gap-4 sm:flex-row sm:items-center rounded-lg bg-muted/50 p-4">
									<!-- Image -->
									...
									<div class="min-w-0 flex-1">
										<div class="flex items-start justify-between gap-2">
											...
											<span class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium {status.class}">
												<status.Icon class="h-3 w-3" /> {status.label}
											</span>
										</div>
										<div class="mt-1 flex gap-4 text-sm">
											<span>Setor: <span class="font-medium">{formatCurrency(product.priceBuy)}</span></span>
											{#if product.status === 'approved'}
												<span>Jual: <span class="font-medium">{formatCurrency(product.priceSell)}</span></span>
											{/if}
										</div>
									</div>
									...
									<div class="flex gap-2 flex-shrink-0">
										{#if product.status === 'pending'}
											<form method="POST" action="?/approve" use:enhance class="flex gap-2 items-end">
												...
												<Button type="submit" size="sm" class="gap-1"><Check class="h-3 w-3" /> Setujui</Button>
											</form>
											<Button size="sm" variant="outline" class="text-destructive" onclick={() => openRejectModal(product.id, product.name)}>
												<X class="h-3 w-3" />
											</Button>
										{/if}
										<Button size="sm" variant="outline" class="text-destructive" onclick={() => openDeleteModal(product.id, product.name)}>
...
{#if rejectModal.open}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
		<Card class="w-full max-w-md">
			...
			<form method="POST" action="?/reject" use:enhance={() => { return async ({ update }) => { closeRejectModal(); await update(); }; }} class="mt-4 space-y-4">
				...
				<div class="space-y-2">
					...
					<textarea id="reason" name="reason" rows="3" class="flex w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" bind:value={rejectReason}></textarea>
				</div>
				<div class="flex gap-3">
					<Button type="button" variant="outline" class="flex-1" onclick={closeRejectModal}>Batal</Button>
...
{#if deleteModal.open}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
		<Card class="w-full max-w-md">
			...
			<p class="mt-2 text-muted-foreground">Produk "{deleteModal.productName}" akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
			<form method="POST" action="?/delete" use:enhance={() => { return async ({ update }) => { closeDeleteModal(); await update(); }; }} class="mt-4 space-y-4">
				<input type="hidden" name="productId" value={deleteModal.productId} />
				<div class="flex gap-3">
					<Button type="button" variant="outline" class="flex-1" onclick={closeDeleteModal}>Batal</Button>
